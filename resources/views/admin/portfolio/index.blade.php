...
<div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
    ...
    <div class="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
        {{-- SEARCH FORM --}}
        <form method="GET" action="{{ route('admin.portfolio.index') }}" class="relative w-full sm:w-64">
...
@if ($portfolios->count() === 0)
    ...
    <p class="text-gray-500 text-center py-10">
        Tidak ada data Portofolio yang tersedia.
    </p>
        ...
        <table class="w-full text-sm text-left text-gray-500 whitespace-nowrap">
            <thead class="text-xs uppercase text-white bg-indigo-600">
                <tr>
                    ...
                    {{-- Kolom Gambar (Biasanya tidak di-sort) --}}
                    <th class="py-3 px-6 font-bold border-r border-indigo-700">
                        Gambar
                    </th>

                    {{-- Kolom Judul (Sortable) --}}
                    <th class="py-3 px-6 font-bold border-r border-indigo-700 cursor-pointer hover:bg-indigo-700 transition"
                        onclick="window.location='{{ $getSortUrl('judul') }}'">
                        <div class="flex items-center">
                            Judul {!! $renderIcon('judul') !!}
                        </div>
                    </th>
            ...
            <tbody>
                ...
                <tr class="border-b border-gray-300 hover:bg-gray-100 transition">
                    ...
                    {{-- Cell Aksi --}}
                    <td class="py-4 px-6 flex justify-center space-x-4 items-center h-28">
                        <a href="{{ route('admin.portfolio.edit', $item->id) }}"
                           class="text-indigo-600 hover:text-indigo-400 transition duration-150 flex items-center">
                        ...
                        <form method="POST" action="{{ route('admin.portfolio.destroy', $item->id) }}">
                            @csrf
                            @method('DELETE')
                            <button class="text-red-600 hover:text-red-400 transition duration-150 flex items-center"
